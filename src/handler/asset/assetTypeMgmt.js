...
export const createAssetType = async (record) => {
    ...
    try {
        ...
        if (recordObj?.templates && recordObj?.templates?.length !== 0) {
            recordObj.templates = recordObj?.templates.map(id => new Types.ObjectId(id));
        }
        ...
        if (recordObj?.parentId && Types.ObjectId.isValid(recordObj?.parentId)) {
            let getResponse = await getAssetTypeCore({
            ...
            let updateResponse = await updateAssetTypeCore(updateObj);
            ...
            let addResponse = await addRecord(recordObj, requiredFields, assetType);
...
export const getAssetTypeDetails = async (record) => {
    const requiredFields = ['companyId'];
    let recordObj = {};
    try {
        ...
        recordObj.nodeId = (record?.queryStringParameters?.nodeId && Types.ObjectId.isValid(record?.queryStringParameters?.nodeId))
            ...
            : null;
        let getResponse = await getAssetTypeCore(recordObj, true);
        log.debug(`get Response -> ${JSON.stringify(getResponse)}`);
        if (!getResponse?.type || getResponse?.payload === null) {
            return apiError(404, { message: 'AssetType Not Found' });
        }
...
export const getAssetTypeCore = async (recordObj, withDetails = false) => {
    try {
        ...
        let response = await getRecord(recordObj, requiredFields, params, assetType);
...
// export const getAssetTypeByTypeCore = async (recordObj) => {
//     try {
...
export const deleteAssetType = async (record) => {
    const requiredFields = ['companyId'];
    let recordObj = {};
    try {
        recordObj._id = record.pathParameters.id;
        ...
        let getResponse = await getAssetTypeCore(recordObj);
        log.debug(`get Response -> ${JSON.stringify(getResponse)}`);
        if (!getResponse?.type || getResponse?.payload === null) {
            return apiError(404, 'AssetType Not Found');
        }
        const params = {
            $and: [
                { companyId: recordObj?.companyId },
        ...
        if (recordObj?.nodeId !== null) {
            ...
            let deleteResponse = await deleteRecord(recordObj, requiredFields, params, assetType);
...
export const updateAssetType = async (record) => {
    try {
        let recordObj = JSON.parse(record['body']);
        const requiredFields = ['companyId'];
        recordObj._id = record.pathParameters.id;
        ...
        if (recordObj?.nodeId && Types.ObjectId.isValid(recordObj?.nodeId)) {
            ...
            log.debug(`Final updateObj---->${JSON.stringify(updateObj)}`);
            updateResponse = await updateAssetTypeCore(updateObj);
            log.debug(`updateResponse -> ${JSON.stringify(updateResponse)}`);
        } else {
            updateResponse = await updateAssetTypeCore(recordObj);
            log.debug(`updateResponse -> ${JSON.stringify(updateResponse)}`);
        }
        if (!updateResponse?.type || updateResponse?.payload === null) {
            return apiError(404, updateResponse?.payload);
...
export const updateAssetTypeCore = async (recordObj) => {
    try {
        const requiredFields = ['companyId'];
        const params = {
            $and: [
        ...
        log.debug(`Params -> ${JSON.stringify(params)}`);
        let updateResponse = await updateRecord(recordObj, requiredFields, params, assetType);
        log.debug(`updateResponse -> ${JSON.stringify(updateResponse)}`);
        return {
            type: true,
...
export const getAssetTypeByCompany = async (record) => {
    const requiredFields = ['companyId'];
    let recordObj = {};
    try {
        recordObj.companyId = record.requestToken.companyId;
        ...
        let getResponse = await getAssetTypeByCompanyCore(recordObj, optionParams);
        log.debug(`get Response -> ${JSON.stringify(getResponse)}`);
        return apiResponse(getResponse?.payload);
    } catch (error) {
        log.debug(`error - ${error}`);
...
export const getAssetTypeByCompanyCore = async (recordObj, optionParams) => {
    try {
        ...
        log.debug(`params -> ${JSON.stringify(params)}`);
        const total = await getRecordCount(params, assetType);
        ...
        let aggrigation = await aggregationAssetTypeRes(params, optionParams?.size, skip);
        log.debug(`aggrigation -> ${JSON.stringify(aggrigation)}`);
        let getResponse = await getRecordWithAggrigation(aggrigation?.payload, assetType);
        log.debug(`Get  Response -> ${JSON.stringify(getResponse)}`);
        return {
            type: true,
            payload: {
...
const aggregationAssetTypeRes = async (params, limit = 0, skip = 0, sort = null) => {
    try {
        let aggregate = [
            { $match: params },
            {
