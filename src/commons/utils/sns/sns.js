...
var sns = new SNS({ region: AppConfig.REGION });
...
export const SNS_ADD_DEFAULT_COMPANY_DATA = async (recordLocal) => {
  var params = {
    Message: JSON.stringify(recordLocal),
    TopicArn: AppConfig.SNS_ADD_DEFAULT_COMPANY_DATA,
  };
  console.log(
    'publishTextPromise SNS_ADD_DEFAULT_COMPANY_DATA  params ===>',
    params,
  );

  let publishTextPromise = await sns.publish(params);
  console.log(
    'publishTextPromise SNS_ADD_DEFAULT_COMPANY_DATA ===>',
    publishTextPromise,
  );
...
export const snsCaller = async (recordLocal, snsName) => {
  log.debug('snsCaller  snsName ===>' + snsName);
  var params = {
    Message: JSON.stringify(recordLocal),
    TopicArn: AppConfig[snsName],
  };
  log.debug('snsCaller  params ===>' + JSON.stringify(params));

  let publishTextPromise = await sns.publish(params);
  log.debug('snsCaller  ===>' + publishTextPromise);
